<template>
  <div class="route-container">
    ...
    <el-card shadow="never" class="route-card">
      <template #header>
        <div class="card-header">
          ...
          <div class="header-controls">
            <el-input v-model="searchQuery" placeholder="搜索航线名称" clearable @clear="handleRouteClea"
      ...
      <el-table :data="filteredRoutes" border stripe v-loading="loading" @row-click="handleRowClick" style="width: 100%"
      ...
      <!-- 点击航信信息弹出航信具体信息 -->
      <el-dialog width="450px" v-model="dialogRouteVisible" title="航线信息详情">
        <el-descriptions :column="1" :size="size" border style="margin-bottom: 15px">
          ...
          <template v-for="(point, index) in routeData.points" :key="index">
            <el-descriptions-item :label="`航点${index + 1}`">
              <div class="point-info">
                <span>经纬度:
                  <span style="color: #9c9b9b">{{ point.lon }}, {{ point.lat }}, 海拔:
                    {{ point.alt }}米</span>
                  |
                </span>
                <span>动作:
                  <span style="color: #9c9b9b">{{ getActionText(point.action) }} |</span>
                </span>
                <span>停留:
                  <span style="color: #9c9b9b">{{ point.residence_time }}秒 |</span>
                </span>
                <span>速度:
                  <span style="color: #9c9b9b">{{ point.velocity }}m/s |</span>
                </span>
                <span>高度策略:
                  <span style="color: #9c9b9b">{{ getHeightStrategyText(point.height_strategy) }} |</span>
                </span>
                <span>航向:
                  <span style="color: #9c9b9b">{{ getHeadingModeText(point.heading_angle?.mode) }} |
                  </span>
                </span>
                <span>分类:
                  <span style="color: #9c9b9b">{{ getCategoryText(point.sort) }} |
                  </span>
                </span>
                <span>优先级:
                  <span style="color: #9c9b9b">{{ getPriorityText(point.priority) }} |</span>
                </span>
                <span>丢失行为:
    ...
    <!-- 编辑航线 -->
    <RouteEdit v-model:visible="editVisible" :route="editingRoute" @save="handleSaveRoute" />
  </div>
</template>
      ...
      <Document>
        ...
        <Placemark>
          ...
          <Point>
            ...
            <coordinates>${homePos.lon},${homePos.lat},${homePos.alt || 0
    }</coordinates>
          </Point>
        </Placemark>
        ...
        <Placemark>
          ...
          <description>
            ...
            终点坐标: ${points[points.length - 1]?.lon}, ${points[points.length - 1]?.lat
    }
          </description>
        </Placemark>
        <!-- 航点标记（完整展示所有字段-中文化） -->
