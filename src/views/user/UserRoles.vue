<template>
  <div class="user-list">
    ...
    >
      ...
      <!-- 操作列 -->
      <!-- 操作按钮（分配菜单、分配资源、编辑、删除）权限：
           1. parentId === null 的超级管理员行：不可点击
           2. 超级管理员 (superAdmin === true) 且非顶级角色：可以点击（包括其他管理员行）
           3. 二级管理员在子级中且不是自己那一行：可以点击
           4. 二级管理员在子级中且是自己那一行：不可点击（自己不能修改自己）
           5. 二级管理员查看其他角色的子级：不可点击
           6. 既不是超级管理员也不是二级管理员：不可点击 -->
      <template #action="{ row }">
        <el-button-group>
