...
const externalNoFlyZones = [
  {
    ...
    "area": 0.45,
    "name": "上海外滩禁飞区",
    "coordinates": "[[[31.2304, 121.4801], [31.2304, 121.487], [31.224, 121.487], [31.225, 121.486], [31.225, 121.4801], [31.2304, 121.4801]]]",
  ...
  shape.addEventListener("mouseover", (e) => {
    ...
    const content = `
      <div style="padding: 8px 12px; min-width: 180px; font-size: 13px; line-height: 1.5;">
        <div style="font-size: 15px; font-weight: 600; color: ${zoneData.borderColor || '#e74c3c'}; margin-bottom: 6px;">
          ${zoneData.name || '未命名禁飞区'}
        </div>
        <div style="margin-bottom: 3px;"><span style="color: #666;">ID：</span>${zoneData.id}</div>
        <div style="margin-bottom: 3px;"><span style="color: #666;">描述：</span>${zoneData.description || '无'}</div>
        <div style="margin-bottom: 3px;"><span style="color: #666;">面积：</span>${areaText}</div>
        <div style="margin-bottom: 3px;"><span style="color: #666;">创建时间：</span>${zoneData.createTime || '无'}</div>
        <div><span style="color: #666;">形状：</span>${zoneData.type === 'polygon' ? '多边形' : '圆形'}</div>
      </div>
    `;
...
const saveNoFlyZoneData = (shape, id) => {
  ...
  if (shape instanceof T.Polygon) {
    ...
    const coordinates = lngLats[0] ? lngLats[0].map((p) => ({ lng: p.lng, lat: p.lat })) : [];
    const area = (calculatePolygonArea(coordinates) / 1000000).toFixed(2);
  ...
  if (stored) {
    ...
    storedZones.forEach((zone) => {
      ...
      if (zone.type === "polygon") {
        ...
        shape = new T.Circle(new T.LngLat(center.lng, center.lat), zone.radius || 1000, {
          color: zone.borderColor || "#e74c3c",
          weight: zone.borderWeight || 2,
          fillColor: zone.fillColor || "#e74c3c",
          fillOpacity: zone.fillOpacity || 0.3,
    ...
    ElMessage.success(`从本地存储加载 ${storedZones.length - externalNoFlyZones.length} 个禁飞区`);
  }
};
  ...
  if (shape instanceof T.Polygon) {
    ...
    points.forEach((point, index) => {
      ...
      marker.addEventListener("dragend", (e) => {
        ...
        if (zoneIndex !== -1) {
          const newCoordinates = points.map(p => ({ lng: p.lng, lat: p.lat }));
          const newArea = (calculatePolygonArea(newCoordinates) / 1000000).toFixed(2);
    ...
    radiusMarker.addEventListener("dragend", (e) => {
      ...
      const zoneIndex = noFlyZones.value.findIndex(z => z.id === shape._zoneId);
      if (zoneIndex !== -1) {
        const newArea = (Math.PI * newRadius * newRadius / 1000000).toFixed(2);
