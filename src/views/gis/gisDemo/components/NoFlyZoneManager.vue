...
const noFlyZonesList = async () => {
  ...
  try {
    ...
    const res = await limitAreaList(data);
    if (res.code === 200) {
      noFlyZones.value = [];
...
const addCircleCenterMarker = (center, editShape, originalShape) => {
  ...
  const zoneData =
    ...
    editShape._zoneData;
  const markerColor = zoneData?.fillColor === "#ffa500" ? "#2c3c54" : "#2c3c54"; // 中心点可以用固定色
...
const finishEditMode = async () => {
  ...
  editMarkers.value.forEach((item) => {
    if (item.editShape && item.editShape._isModified) {
      // 如果是圆形，确保半径标记位置正确
      if (item.editShape instanceof AMap.Circle) {
