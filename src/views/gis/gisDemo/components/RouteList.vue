<template>
  <div class="route-list-container">
    ...
    <div class="route-search">
      <el-input v-model="searchKeyword" @input="handleSearchInput" clearable placeholder="请输入要搜索的航线" />
    </div>
    <el-button style="background: #2c3d45;
    color: #fff;
    margin-bottom: 10px;" v-show='backShow' @click="returnPending">返回代办</el-button>
    <!-- 航线列表 -->
    <div class="route-list-content">
      <div v-for="(item, index) in routeInfo" :key="index">
        <div class="routeOperation" v-for="(itemInfo, indexInfo) in item" :key="indexInfo">
          <div class="routeOperation-box">
            ...
            <div style="
              ...
              <div>
                ...
                <!-- 完成 → 仅编辑时显示 -->
                <span v-if="itemInfo._editing" class="routeOperation-box-complete" @click="completeRouteEdit(itemInfo)">
                  完成
                </span>
          ...
          <!-- 航点列表 -->
          <div class="routeInformation" v-if="itemInfo._editing" :key="`route-${itemInfo.id}`">
            <div class="routeInformation-list" v-for="(itemPrototype, indexPrototype) in getCurrentPoints(
              itemInfo,
            )" :key="`point-${indexPrototype}-${JSON.stringify(itemPrototype)}`" :draggable="itemInfo._editing"
              @dragstart="handleDragStart(indexPrototype, itemInfo)" @dragover="handleDragOver"
              @drop="handleDrop(indexPrototype, itemInfo)" @dragend="handleDragEnd" :class="{ dragging: isDragging }">
              航点 {{ indexPrototype + 1 }}
              {{
                indexPrototype === 0
                  ? "(起点)"
              ...
              <div v-show="indexPrototype !== getCurrentPoints(itemInfo).length - 1
                " class="routeInformation-list-edit" @click="editAirline(itemPrototype, indexPrototype + 1)">
                编辑
              </div>
            </div>
          </div>
    ...
    <el-dialog v-model="waypointSettingVisible" title="航点设置" width="25%" type="warning"
      ...
      <el-form :model="formData" label-width="100px" style="
        ...
        <div style="flex: 1; overflow-y: auto; margin: 10px 0">
          <el-form-item label="航点序号:">
            <el-input disabled v-model="formData.waypointNumber" placeholder="请输入航点序号" />
          </el-form-item>
          <el-form-item label="经度(度):">
            <el-input v-model="formData.lon" type="number" step="0.00001" precision="5" />
          </el-form-item>
          <el-form-item label="纬度(度):">
            <el-input v-model="formData.lat" type="number" step="0.00001" precision="5" />
          </el-form-item>
          <el-form-item label="停留时间" prop="residenceTime">
            <el-input v-model="formData.residence_time" placeholder="请输入停留时间" type="number" :formatter="formatNumber"
              :parser="parseNumber">
              <template #suffix>s</template>
            </el-input>
          </el-form-item>
          <el-form-item label="动作:">
            <el-select v-model="formData.action" placeholder="请选择航向角类型" style="width: 100%">
              <el-option v-for="item in headingActionOptions" :key="item.value" :label="item.label"
                :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="速度:">
            <el-input v-model="formData.velocity" placeholder="请输入速度" type="number" :formatter="formatNumber"
              :parser="parseNumber">
              <template #suffix>m/s</template>
            </el-input>
          </el-form-item>
          <el-form-item label="航向角:">
            <el-select v-model="formData.heading_angle.mode" placeholder="请选择航向角类型" style="width: 100%">
              <el-option v-for="item in headingAngleOptions" :key="item.value" :label="item.label"
                :value="item.value" />
            </el-select>
            <el-input v-show="formData.heading_angle.mode ===
              'at an angle to the target point'
              " v-model="formData.heading_angle.angle" style="width: 100%; margin-top: 10px" placeholder="请输入与目标点的角度"
              type="number" min="0" max="360">
              <template #suffix>°</template>
            </el-input>
            <div v-show="formData.heading_angle.mode === 'towards a certain point'" class="coordinate-inputs">
              <el-input v-model="formData.heading_angle.lon" style="width: 50%; margin-top: 10px" placeholder="请输入经度"
                type="number" step="0.000001" min="0">
                <template #suffix>°</template>
              </el-input>
              <el-input v-model="formData.heading_angle.lat" style="width: 50%; margin-top: 10px" placeholder="请输入纬度"
                type="number" step="0.000001" min="0">
                <template #suffix>°</template>
              </el-input>
            </div>
          ...
          <el-form-item label="高度策略:">
            <el-select v-model="formData.height_strategy" placeholder="请选择高度策略类型" style="width: 100%">
              <el-option v-for="item in heightStrategyOptions" :key="item.value" :label="item.label"
                :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="优先级:">
            <el-select v-model="formData.priority" placeholder="请选择优先级类型" style="width: 100%">
              <el-option v-for="item in priorityOptions" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="分类:">
            <el-select v-model="formData.sort" placeholder="请选择分类类型" style="width: 100%">
              <el-option v-for="item in categoryOptions" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="航线失联行为:">
            <el-select v-model="formData.route_loss_behavior" placeholder="请选择航线失联行为" style="width: 100%">
              <el-option v-for="item in lossBehaviorOptions" :key="item.value" :label="item.label"
                :value="item.value" />
            </el-select>
          </el-form-item>
        </div>
    ...
    <!-- 保存路线弹窗 -->
    <el-dialog v-model="saveRouteDialogVisible" :title="dialogTitle" width="30%" type="warning"
      style="max-height: 70%; overflow: hidden">
      <el-form ref="saveRouteFormRef" :model="saveRouteForm" :rules="saveRouteRules" label-width="90px" style="
          max-height: calc(70vh - 120px);
          display: flex;
          flex-direction: column;
        ...
        <div class="waypoints-scroll-container" style="flex: 1; overflow-y: auto; margin: 10px 0">
          <div class="save-route" v-for="(item, index) in saveRouteForm.points" :key="index"
...
const saveRouteRules = {
  ...
  waypointStrategy: [
    { required: true, message: "请选择航点策略", trigger: "change" },
  ],
};
...
const routeEdit = async (route) => {
  // 从routeInfo中获取最新的route引用
  const latestRoute = routeInfo.value[0]?.find((item) => item.id === route.id);
  ...
  latestRoute._editing = true;
  latestRoute._expanded = true;

  activeRouteId.value = latestRoute.id;
  isEditMode.value = true;
  editingRouteId.value = latestRoute.id;
  isEditing.value = true;

  hasDragged.value[latestRoute.id] = false;
  // 始终用最新的routeInfo数据刷新缓存，避免编辑路线保存后旧数据残留
  originalPoints.value[latestRoute.id] = JSON.parse(
    JSON.stringify(latestRoute.points || []),
  );
  dragTempPoints.value[latestRoute.id] = JSON.parse(
    JSON.stringify(latestRoute.points || []),
  );
  isEditCompleted.value = false;

  emit("route-edit", latestRoute);
};
...
const completeRouteEdit = async (route) => {
  ...
  try {
    ...
    // 使用convertPoints转换，保留所有航点属性（action、velocity、headingAngle等）
    const pointsJson = convertPoints(finalPoints);
    ...
    if (res.code === 200) {
      ...
      // 清除旧的临时数据，确保下次编辑时从最新数据重建
      delete dragTempPoints.value[route.id];

      ElMessage.success("航线保存成功");
      await routeList(searchKeyword.value);
      // routeList刷新后routeInfo已更新，从最新的routeInfo中获取route引用
      const latestRoute = routeInfo.value[0]?.find((item) => item.id === route.id);
      emit("route-saved-and-refresh-map", latestRoute?.id || route.id);
      emit("route-save");
    }
  } catch (error) {
    console.error(error);
    ...
    if (valid) {
      ...
      if (dialogTitle.value === "保存路线") {
        try {
          ...
          newArr.value = {
            name: saveRouteForm.value.namereplace(/\s/g, ''),
        ...
        newArr.value = {
          name: saveRouteForm.value.name.replace(/\s/g, ''),
          description: saveRouteForm.value.description,
        ...
        if (res.code === 200) {
          ...
          // 清除旧的缓存数据，确保下次编辑时从最新数据重建
          delete originalPoints.value[listRouteEditId.value];
          delete dragTempPoints.value[listRouteEditId.value];
          await routeList(searchKeyword.value);
          emit("route-save");
        }
      }
