...
import { todoSummary } from "@/api/todo.js";
...
// 注入代办数量，用于同步首页头像badge
const unreadRef = inject("unreadRef", null);
...
const initMap = () => {
  ...
  try {
    ...
    map.value.on("complete", () => {
      ...
      // 加载所有需要的插件
      AMap.plugin(["AMap.Geocoder", "AMap.PlaceSearch", "AMap.Scale"], () => {
    ...
    if (valid) {
      ...
      if (dialogTitle.value === "保存路线") {
        try {
          ...
          if (!policyIdDialog.value && waypointOptions.value.length > 0) {
...
// 新增：处理完成编辑事件
const handleRouteCompleted = async (routeId) => {
  // 清除地图上的航线覆盖物
  clearRouteOverlaysOnly();
  isEditingRoute.value = false;
  activeRouteId.value = null;
  currentRoutePolyline.value = null;
  // 如果从冲突代办进入，编辑完成后刷新代办数量
  if (route.query.source) {
    try {
      const res = await todoSummary();
      if (res.code === 200) {
        const count = res.data.unreadCount;
        // 同步首页头像badge
        if (unreadRef) unreadRef.value = count;
        localStorage.setItem("unreadCount", count);
        if (count > 0) {
          ElMessage.warning(`您还有 ${count} 条代办需要处理`);
        } else {
          ElMessage.success("所有代办已处理完毕");
        }
      }
    } catch (err) {
      console.error("刷新代办数量失败:", err);
    }
  }
};
...
// 监听 RouteList 挂载完成，有 source 参数时自动触发搜索
watch(routeListRef, (newVal) => {
  if (newVal && route.query.source) {
    newVal.routeList(String(route.query.source));
  }
});
// 初始化
onMounted(async () => {
  (window.__amapReady || Promise.resolve()).then(() => {
    initMap();
    // 从 todo 页面跳转过来时，自动触发搜索（由 watch routeListRef 执行）
  });
  await getAllPolicies();
  //去掉父级继承的padding
