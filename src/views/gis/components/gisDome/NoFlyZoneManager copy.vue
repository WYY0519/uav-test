<template>
  ...
  <div class="control-toolbar" v-show="true">
    <h3>区域管理</h3>
    <button
  ...
  >
    <div class="tooltip-header">
      <span
        :class="{
          tooltipTitlejf: tooltipData.fillColor === '#e74c3c',
          tooltipTitlejg: tooltipData.fillColor === '#ffa500',
        }"
      >
        {{
          tooltipData.fillColor === "#e74c3c" ? "禁飞区详情" : "警告区详情"
        }}</span
      >
    </div>
    <div class="tooltip-content">
      <p><span class="label">名称：</span>{{ tooltipData.name || "未命名" }}</p>
      <p>
        <span class="label">类型：</span
        >{{ tooltipData.type === "polygon" ? "多边形" : "圆形" }}
      </p>
      <p>
        <span class="label">描述：</span>{{ tooltipData.description || "无" }}
      </p>
      <p>
        <span class="label">面积：</span>
        {{ formatArea(tooltipData.area) }}
      </p>
      <p v-if="tooltipData.type === 'circle'">
        <span class="label">半径：</span>{{ formatRadius(tooltipData.radius) }}
      </p>
    </div>
  </div>
</template>
...
const isPointInCircle = (point, circle) => {
  ...
  return distance <= radius + 1e-5; // 浮点误差容错
};
...
// 优化航线穿越检查：结合采样点+线段相交检测
// 检查路线是否穿越禁飞区（只检查禁飞区，不检查警告区）
    ...
    for (const zone of noFlyZonesOnly) {
      ...
      if (
        ...
        if (discriminant >= 0) {
          ...
          // 检查是否有交点在线段内部（0<t<1）
          const hasInternalIntersection =
            (t1 > 0 && t1 < 1) || (t2 > 0 && t2 < 1);

          if (hasInternalIntersection) {
            console.log(`航线段 ${i}->${i + 1} 与圆形禁飞区 ${zone.name} 相交`);
            return true;
          }
        }
      }
...
// 在 NoFlyZoneManager.vue 中添加警告区检查方法
...
const isPointInWarningZone = (point) => {
  ...
  for (let i = 0; i < noFlyZones.value.length; i++) {
    const zone = noFlyZones.value[i];
    if (!zone || !zone.coordinates || zone.regionType !== "jg") {
      continue; // 只检查警告区
    }
...
const exposeMethods = {
  isPointInNoFlyZone, // 检查点是否在禁飞区
  isRouteCrossingNoFlyZone, // 检查航线是否穿越禁飞区
  isRouteCrossingWarningZone, // 检查航线是否穿越警告区
  getNoFlyZones: () => noFlyZones.value,
  reinitialize, // 暴露重新初始化方法
  refreshNoFlyZones, // 添加刷新方法
};
...
const tooltipStyle = computed(() => ({
  left: `${mousePosition.value.x - 130}px`,
  top: `${mousePosition.value.y - 160}px`,
  ...
  formRef.value.validate(async (valid) => {
    if (valid && tempDrawData.value) {
      try {
        const currentColorValue = currentColor.value;
        const apiParams = {
          area: tempDrawData.value.area.toFixed(3),
...
const enterEditMode = async () => {
  ...
  const allShapes = props.map
    .getOverlays()
    .filter((overlay) => overlay._zoneId)
    .filter((shape) => {
      // 只处理多边形和圆形
      const isValidShape =
      ...
      // 只处理当前选中类型的区域
      const zoneData =
        shape._zoneData || noFlyZones.value.find((z) => z.id === shape._zoneId);
  ...
  allShapes.forEach((shape) => {
    ...
    if (zoneData) {
      ...
      // 移除所有相关的标记点
      removeRelatedMarkers(shape._zoneId);
...
// 根据禁飞区ID移除相关标记点
const removeRelatedMarkers = (zoneId) => {
  const allOverlays = props.map.getOverlays();
  allOverlays.forEach((overlay) => {
    if (overlay instanceof T.Marker && overlay._zoneId === zoneId) {
      props.map.removeOverLay(overlay);
      if (noFlyZonesLayer.value) {
...
const createEditShape = (originalShape) => {
  ...
  if (editShape) {
    // 保存必要的数据
    editShape._zoneId = originalShape._zoneId;
    editShape._zoneData = { ...zoneData };
    ...
    existingOverlays.forEach((overlay) => {
      if (overlay._isEditShape && overlay._zoneId === editShape._zoneId) {
        props.map.removeOverLay(overlay);
        if (noFlyZonesLayer.value) {
          noFlyZonesLayer.value.removeLayer(overlay);
        }
...
const addCircleCenterMarker = (center, editShape, originalShape) => {
  ...
  marker._zoneId = originalShape._zoneId;
  ...
  marker.addEventListener("dragstart", () => {
    ...
    const radiusMarkerItems = editMarkers.value.filter(
      (item) => item.zoneId === marker._zoneId && item.type === "circle_radius"
    );
    ...
    editMarkers.value = editMarkers.value.filter(
      (item) =>
        !(item.zoneId === marker._zoneId && item.type === "circle_radius")
    );
  });
  ...
  marker.addEventListener("dragend", (e) => {
    ...
    editMarkers.value.push({
      ...
      originalShape,
      type: "circle_radius",
      zoneId: marker._zoneId, // 使用 marker 上的 _zoneId 确保一致性
    });
...
// 添加多边形顶点标记
// 修改 addPolygonVertexMarker 函数，给 color 参数一个默认值
// 修改 addPolygonVertexMarker 函数，不依赖外部传入的 color 参数
...
const finishEditMode = async () => {
    ...
    if (item.editShape && item.editShape._isModified) {
      ...
      if (item.editShape instanceof T.Circle) {
        const radiusMarker = editMarkers.value.find(
          (m) => m.zoneId === item.zoneId && m.type === "circle_radius"
        );
  ...
  if (modifiedZones.length === 0) {
    ...
    ElMessage.info("未修改任何禁飞区，已退出编辑模式");
    return;
  }
  ...
  try {
    ...
    if (allSuccess) {
      ElMessage.success(`成功更新 ${modifiedZones.length} 个禁飞区`);
      // 重新加载禁飞区列表
      await noFlyZonesList();
...
const restoreOriginalShapes = () => {
  ...
  originalShapes.forEach((shape) => {
    ...
    if (shape instanceof T.Polygon || shape instanceof T.Circle) {
      ...
      const existingOverlays = props.map.getOverlays();
      existingOverlays.forEach((overlay) => {
        if (overlay._zoneId === shape._zoneId && overlay !== shape) {
          props.map.removeOverLay(overlay);
          if (noFlyZonesLayer.value) {
            noFlyZonesLayer.value.removeLayer(overlay);
          }
...
const clearEditMarkers = () => {
  ...
  allOverlays.forEach((overlay) => {
    if (
      ...
      (overlay._isEditMarker || !overlay._zoneId)
    ) {
      props.map.removeOverLay(overlay);
    }
  });
...
const restoreAllZonesStyle = () => {
  ...
  allOverlays.forEach((overlay) => {
    const zoneData =
      overlay._zoneData ||
      noFlyZones.value.find((z) => z.id === overlay._zoneId);
...
const handleNoFlyZoneClick = function () {
  const overlay = this;
  const zoneId = overlay._zoneId;

  // 找到对应的禁飞区数据
  const zone = noFlyZones.value.find((z) => z.id === zoneId);
  ...
  const zoneDisplayName = zone.name || `${zoneTypeName}${zoneId}`;
  ...
  ElMessageBox.confirm(
    ...
    .then(async () => {
      try {
        const res = await noflyzoneDelete(zoneId);
        if (res.code === 200) {
          // 从图层移除
          if (noFlyZonesLayer.value) {
            noFlyZonesLayer.value.removeLayer(overlay);
    ...
    .then(async () => {
      try {
        ...
        const allOverlays = props.map.getOverlays().filter((overlay) => {
          if (!overlay._zoneId && !overlay._isNoFlyZone) return false;
...
const bindTooltipEvents = (shape, zoneData) => {
  shape.addEventListener("click", (e) => {
    ...
    if (zoneData.type === "polygon") {
      mousePosition.value.x = e.containerPoint.x;
      mousePosition.value.y = e.containerPoint.y;
    } else {
      mousePosition.value.x = e.containerPoint.x;
      mousePosition.value.y = e.containerPoint.y - 20;
    }
...
const formatArea = (area) => {
  if (!area) return "计算中...";
  return area >= 1
    ? area.toFixed(2) + (area > 1 ? " 平方公里" : " 平方米")
    : (area * 1000000).toFixed(2) + " 平方米";
};
