...
const initMap = () => {
  ...
  try {
    ...
    map.centerAndZoom(new T.LngLat(defaultLon, defaultLat), 15);

    // 默认显示飞机图标
    markPositionOnMap2(defaultLon, defaultLat, 30);
      ...
      if (res.data.status) {
        ...
        if (res.data.statusData.global_position_int) {
          ...
          if (lon !== 0 && lat !== 0 && !isNaN(lon) && !isNaN(lat)) {
            ...
            // 经纬度为0或无效时，使用北京市默认经纬度
            console.log("经纬度为默认0值，使用北京市默认位置");
            ...
            // 显示小飞机在默认位置
            droneHeading.value = 45; // 初始航向设为45度
            markPositionOnMap2(defaultLon, defaultLat, alt || 2);
          }
        }
      } else {
        // 当没有状态数据时，使用北京市默认经纬度
        console.log("无状态数据，使用北京市默认位置");
        const defaultLon = 116.397428;
        const defaultLat = 39.90923;

        // 初始化航向
        droneHeading.value = 45;
