  ...
  <div class="drone-monitor">
    ...
    <div class="top-panel">
      <el-input style="padding: 12px" v-model="searchQuery" placeholder="输入无人机编号搜索" class="search-input" clearable
        ...
        <template #append>
          <el-button style="background-color: #409eff; color: white; padding-left: 17px" :icon="Search"
            @click="handleSearch" />
        </template>
      </el-input>
    </div>
      ...
      <div class="panel-content">
        <div class="monitor-section">
          ...
          <div v-if="droneMonitoringShow" class="webrtc-player-container">
            <video style="width:100%;height:100%" ref="videoEl" autoplay muted playsinline controls></video>
        ...
        <div class="mission-section">
          <div class="mission-buttons" style="
            ...
            <div class="upload-container">
              <el-input v-model="fileName" placeholder="请输入航线名称" style="height: 26px; margin-bottom: 8px" clearable />
        ...
        <div class="main-controls">
          ...
          <el-button type="info" class="control-btn" :disabled="!isConnected || !canOperate">
            <div class="lock-slider-container" :class="{ disabled: !isConnected }">
              <div class="lock-slider" :class="{ locked: isLocked, 'cursor-not-allowed': isLocked }"
                @mousedown="startDrag" @touchstart="startDrag" ref="sliderContainer">
                <div class="slider-thumb" :style="{ transform: `translateX(${thumbPosition}px)` }" ref="sliderThumb">
                  <el-icon>
                    <Right />
                  </el-icon>
                </div>
          ...
          <el-button type="info" class="control-btn" @click="handleReturnLand" :disabled="!isConnected || !canOperate">
            <el-icon>
              <Bottom />
            </el-icon>
            降落
...
">
        ...
        <div class="direction-controls">
          <div class="direction-buttons bottom-buttons">
            <div style="margin-bottom: 6px">
              ...
              <el-button :disabled="!isConnected || !canOperate" @click="handleDirection1('up')">
                <img :src="ICONS.forward" alt="" </el-button>
            </div>
            <div style="display: flex">
              <div style="text-align: center; margin-right: 6px">
                <el-button :disabled="!isConnected || !canOperate" @click="handleDirection1('left')">
                  <img :src="ICONS.theLeft" alt="" />
                </el-button>
                <div style="margin-top: 6px">左</div>
              </div>
              <div style="text-align: center">
                <el-button :disabled="!isConnected || !canOperate" @click="handleDirection1('down')">
                  <img :src="ICONS.backward" alt="" />
                </el-button>
                <div style="margin-top: 6px">下</div>
              </div>
              <div style="text-align: center; margin-left: 6px">
                <el-button :disabled="!isConnected || !canOperate" @click="handleDirection1('right')">
                  <img :src="ICONS.theRight" alt="" />
                </el-button>
        ...
        <div class="functionButtons">
          <el-button class="Button" style="margin-left: 12px" type="info" @click="handleDirection1('connect')"
            :disabled="!isConnected || !canOperate">
            <i class="el-icon-video-pause"></i> 连接
          </el-button>
          <el-button class="Button" type="info" @click="handleDirection1('stop')"
            :disabled="!isConnected || !canOperate">
            <i class="el-icon-video-pause"></i> 暂停
          </el-button>
          <el-button class="Button" type="info" @click="handleDirection1('home')"
            :disabled="!isConnected || !canOperate">
            <i class="el-icon-location"></i> 归中
          </el-button>
          <el-button class="Button" type="info" @click="handleDirection1('lowermost')"
            :disabled="!isConnected || !canOperate">
    ...
    <!-- 查看详情弹窗组件 -->
    <UavMonitorVisible v-model="isDetailDialogVisible" :uav-status-content="uavStatusContent" />
    ...
    <!-- 上传航线 -->
    <el-dialog class="uploadRoute" title="上传航线" v-model="returnVoyageDialogVisibleUploadRoute" width="600px"
      max-height="600px" destroy-on-close>
      <el-table :data="filteredRoutes" border stripe v-loading="loading" @row-click="handleRowClick"
        style="width: 100%; height: 400px" :header-cell-style="{ background: '#f5f7fa', color: '#606266' }">
        <el-table-column width="55">
          <template #default="{ row }">
            <el-radio v-model="selectedRouteId" :label="row?.id" @change="() => handleRadioChange(row)">
              <span></span>
            </el-radio>
          </template>
        </el-table-column>
...
const markPositionOnMap2 = (lng, lat, height) => {
  ...
  try {
    ...
    if (dronePositionMarker.value) {
      try {
        // 更新位置
        dronePositionMarker.value.setPosition(new AMap.LngLat(lng, lat));
...
">
  ...
  "></div>
        ...
        dronePositionMarker.value.setContent(html);
  ...
  (newVal) => {
    ...
    if (map && typeof map.resize === "function") {
      ...
      setTimeout(() => {
        ...
        // 如果有必要，可以同时调整地图视野
        if (planeRealTimePos && planeRealTimePos.lon && planeRealTimePos.lat) {
          map.panTo(
            new AMap.LngLat(planeRealTimePos.lon, planeRealTimePos.lat)
          );
        }
      }, 300);
    }
...
const markRouteOnMap = (routeData) => {
  ...
  try {
    ...
    // 2. 先过滤无效坐标，并转换字符串类型为数字类型（关键修复）
    const validRoutePoints = routeData.map((point) => {
      // 转换字符串为数字类型（兼容数据2中数值为字符串的情况）
      const lat = parseFloat(point.lat);
      const lon = parseFloat(point.lon);
      const alt = parseFloat(point.alt) || 30; // 默认高度30米
    ...
    }).filter(point => point !== null); // 移除null值
    ...
    // 注意：validRoutePoints已经是转换后的数字类型，直接使用即可
...
const drawRouteLine = (routePoints) => {
  ...
  try {
    // 创建新航线（高德地图不支持arrowheads属性，移除该属性）
...
const fitMapToRoute = (routePoints) => {
  ...
  try {
    ...
    console.log(
      `地图视野已自动适配，显示 ${routePoints.length} 个航点，边界: [${minLng}, ${minLat}] - [${maxLng}, ${maxLat}]`
    );
  } catch (viewErr) {
    console.error("使用 setBounds 失败，尝试备用方案：", viewErr);

    // 备用方案：使用 setFitView 方法
    try {
      const tempMarkers = routePoints.map((point) => {
        return new AMap.Marker({
          position: point,
          visible: false, // 隐藏临时标记
        });
      });

      // 添加临时标记到地图
      tempMarkers.forEach((marker) => map.add(marker));

      // 自动调整地图视野以显示所有标记
      map.setFitView();

      // 清除临时标记
      tempMarkers.forEach((marker) => map.remove(marker));

      console.log(`备用方案：使用 setFitView 成功`);
    } catch (fitViewErr) {
      console.error("备用方案也失败了，使用手动计算：", fitViewErr);
      ...
      try {
        ...
        let zoomLevel = calculateOptimalZoom(lngSpan * 1.3, latSpan * 1.3);

        map.panTo(centerPoint);
        map.setZoom(zoomLevel);

        console.log(`最后方案：中心点 (${centerLng}, ${centerLat}), 缩放 ${zoomLevel}`);
      } catch (backupErr) {
        console.error("所有方案都失败了：", backupErr);
      ...
      if (res.data.status) {
        ...
        if (dronePositionMarker.value && map) {
          try {
            ...
            console.warn("清除小飞机标记失败:", removeError);
          }
        }
      }
    }
...
const aaaaa = async (value) => {
  if (value === 1) {
    let res = await dronesDrones()
    console.log(res, "11111");
  } else {
    let res = await dronesStop()
